...
(function () {
  ...
  async function generateChords(artist, title) {
    ...
    const system =
      ...
      "STEP 2 web_fetch로 그 페이지를 '직접 열어' 본문의 실제 가사를 읽는다. 가사는 반드시 이 fetch한 본문에서만 가져온다. 검색 스니펫만으로 가사를 쓰지 마라.\n" +
      "STEP 3 코드 처리: (a) 코드가 가사에 정렬된 실제 차트면 그대로 전사 (b) 코드 목록만 있으면 fetch한 실제 가사에 배치 (c) 코드 자료가 없으면 fetch한 '실제 가사' 위에 원키 화성으로 코드를 직접 얹는다. 어느 경우든 '가사 자체는 항상 fetch한 실제 가사'다.\n" +
      "STEP 4 (출처, 정직): transcribed=실제 코드차트(코드+가사정렬) 전사 / chords_verified=실제 코드와 실제 가사 확인, 배치는 추정 / chords_derived=가사는 실제(fetch), 코드는 이론으로 얹음.\n" +
