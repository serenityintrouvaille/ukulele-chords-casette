...
(function () {
  ...
  async function identifySong(artist, title) {
    const system =
      ...
      "- year·album은 검색 결과에서 확인된 값만 적는다. 확인 안 되면 빈 문자열. 추측한 연도·앨범·러닝타임을 지어내지 마라.\n" +
      "- sourceUrl: 곡 존재를 확인한 실제 페이지 URL. 이게 없으면(=실제로 못 찾았으면) found=false 여야 한다.\n" +
      "- 조금이라도 불확실하면 found=false. 없는 곡을 있다고 하지 마라.\n" +
      '출력은 JSON 1개뿐(설명·펜스 금지): {"found":boolean,"title":"","artist":"","year":"","album":"","sourceUrl":"","note":""}';
    const user = `가수: ${artist || "(미입력)"}\n곡명: ${title || "(미입력)"}\nweb_search로 이 곡이 실제 존재하는지 확인하고, 확인된 정보만 채워줘. 못 찾으면 found=false.`;
    const text = await callClaude(system, user, { maxTokens: 900, timeoutMs: 90000, useSearch: true, effort: "medium", maxUses: 3 });
  ...
  async function generateChords(artist, title) {
    ...
    const system =
      "너는 우쿨렐레 편곡자다.\n" +
      "[가장 중요 — 절대 규칙] 곡·가사·메타데이터를 절대 지어내지 마라. 모든 가사는 web_fetch로 '직접 연 실제 페이지'에서 그대로 가져와야 한다. 네 기억으로 가사를 한 줄이라도 쓰는 것은 금지. 실제 가사를 못 구하면 가사를 만들지 말고 notFound로 답하라.\n" +
      "[알고리즘 — 실제 코드악보 우선]\n" +
      "STEP 1 web_search로 '[가수] [곡명] 코드', '[가수] [곡명] 기타코드', '[가수] [곡명] 우쿨렐레 코드', '[가수] [곡명] chords'를 검색해 '코드가 가사 위에 정렬된 실제 코드 악보' 페이지를 우선 찾는다(코드 사이트·기타/통기타 블로그·Ultimate Guitar·Chordie 등). 코드 차트가 안 보이면 그 다음에 가사 페이지를 찾는다.\n" +
      "STEP 2 web_fetch로 그 페이지를 '직접 열어' 본문에서 가사와 '각 구절의 코드 변화'를 읽는다. 가사·코드 모두 fetch한 본문에서만 가져온다(검색 스니펫 금지).\n" +
      "STEP 3 코드 배치 (가장 중요):\n" +
      "  - 한 곡의 코드는 한 개로 고정이 아니다. 한 구절(한 줄) 안에서도 보통 코드가 1~3번 바뀐다. 실제 악보의 코드 변화 지점을 '그 음절 앞'에 [코드]로 그대로 옮긴다.\n" +
      "  - 같은 코드를 모든 줄에 반복하거나, 줄 맨 앞에 코드 1개만 몰아넣지 마라. 구절마다 코드가 달라지는 것이 정상이다.\n" +
      "  - (a) 코드가 가사에 정렬된 실제 차트면 그 정렬을 그대로 전사한다. (b) 코드 목록만 있으면 곡 구조에 맞게 구절별로 배치한다. (c) 코드 자료가 전혀 없으면 fetch한 '실제 가사' 위에 원키 화성으로 '구절마다 바뀌는' 코드를 얹는다(한 코드로 때우지 말 것). 어느 경우든 가사 자체는 항상 fetch한 실제 가사다.\n" +
      "STEP 4 (출처, 정직): transcribed=코드가 가사에 정렬된 실제 차트를 그대로 전사 / chords_verified=실제 코드는 확인했으나 어느 음절에 얹히는지는 추정 / chords_derived=코드 자료가 없어 실제 가사 위에 코드를 이론으로 얹음. 실제 차트를 찾으면 반드시 transcribed를 우선하라(추측보다 전사가 항상 낫다).\n" +
      "[필수] lyricsSourceUrl 에는 가사를 가져온 실제 페이지 URL을 넣는다. 실제 가사를 web_fetch로 확인하지 못했으면 다른 필드 없이 {\"notFound\":true} 만 출력한다. (가사를 지어내는 것보다 notFound가 항상 낫다.)\n" +
      "[우쿨렐레 적합] 원키 유지. 흔한 키(C·G·F·D·A) 코드 선호. 코드명만 적고 운지(프렛)는 절대 적지 마라(앱이 계산).\n" +
      ...
      "실제 가사를 못 찾으면 {\"notFound\":true} 만 출력.";
    ...
    const text = await callClaude(system, user, { maxTokens: 8000, timeoutMs: 240000, useSearch: true, effort: q.effort, maxUses: Math.max(q.maxUses, 4) });
    return parseJson(text);
  }
  ...
  $("searchForm").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      pendingSong = { title: song.title, artist: song.artist, year: song.year, album: song.album, note: song.note, sourceUrl: song.sourceUrl };
  ...
  function renderConfirm(song) {
    ...
    card.append(q, t, a);
    const extraText = [song.year, song.album, song.note].filter(Boolean).join(" · ");
    if (extraText) {
      const ex = document.createElement("div"); ex.className = "song-extra"; ex.textContent = extraText;
      card.appendChild(ex);
    }
    // 실제 곡인지 확인할 수 있는 출처 링크
    if (song.sourceUrl) {
      const src = document.createElement("div"); src.className = "song-extra";
      const a = document.createElement("a"); a.href = song.sourceUrl; a.target = "_blank"; a.rel = "noopener";
      a.textContent = "🔗 곡 정보 출처 확인";
      src.appendChild(a); card.appendChild(src);
    }
  ...
  async function loadChords(song) {
    ...
    try {
      ...
      if (data.notFound || !data.sections || !data.sections.length) {
        toast("이 곡의 실제 가사/코드를 웹에서 찾지 못해 악보를 만들 수 없어요 (지어내지 않아요)", 3600);
        showScreen("search");
        return;
      }
  ...
  function renderResult(data) {
    ...
    if (data.source === "transcribed") {
      ...
      badge.textContent = "🎼 가사는 실제(웹 출처) · 코드는 직접 편곡 — 코드 자료가 없어 원키 화성으로 코드를 얹었어요. 참고용이에요";
    }
    // 가사·코드 출처 링크(실제 페이지 확인용)
    const srcUrl = data.lyricsSourceUrl || data.sourceUrl;
    if (srcUrl) {
      const a = document.createElement("a"); a.href = srcUrl; a.target = "_blank"; a.rel = "noopener";
    ...
    (data.sections || []).forEach((sec) => {
      ...
      (sec.lines || []).forEach((line) => {
        try { block.appendChild(renderLyricLine(normalizeLyricLine(line))); } catch (_) {}
      });
  ...
  // 모델이 line을 문자열이 아닌 객체/배열로 줄 때도 "[코드]가사" 문자열로 정규화 (렌더가 깨지지 않게)
  function normalizeLyricLine(line) {
    if (typeof line === "string") return line;
    if (line == null) return "";
    if (Array.isArray(line)) return line.map(normalizeLyricLine).join("");
    if (typeof line === "object") {
      const rawChord = line.chord || line.chords || line.c || "";
      const chord = Array.isArray(rawChord) ? rawChord.join("") : String(rawChord);
      const text = line.text || line.lyric || line.lyrics || line.word || line.w || "";
      const c = chord.replace(/^\[|\]$/g, "").trim();
      return (c ? "[" + c + "]" : "") + String(text);
    }
    return String(line);
  }
  ...
  function renderLyricLine(line) {
    ...
    if (typeof line !== "string") line = normalizeLyricLine(line);
