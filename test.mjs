...
async function generateChords(artist, title) {
  const system =
    ...
    "[알고리즘 — 실제 코드악보 우선]\n" +
    "STEP 1 web_search로 '[가수] [곡명] 코드', '[가수] [곡명] 기타코드', '[가수] [곡명] 우쿨렐레 코드', '[가수] [곡명] chords'를 검색해 '코드가 가사 위에 정렬된 실제 코드 악보' 페이지를 우선 찾는다(코드 사이트·기타/통기타 블로그·Ultimate Guitar·Chordie 등). 없으면 그 다음에 가사 페이지를 찾는다.\n" +
    "STEP 2 web_fetch로 그 페이지를 '직접 열어' 본문에서 가사와 '각 구절의 코드 변화'를 읽는다. 가사·코드 모두 fetch한 본문에서만 가져온다(스니펫 금지).\n" +
    "STEP 3 코드 배치 (가장 중요): 한 곡의 코드는 한 개로 고정이 아니다. 한 구절(한 줄) 안에서도 보통 코드가 1~3번 바뀐다. 실제 악보의 코드 변화 지점을 '그 음절 앞'에 [코드]로 옮긴다. 같은 코드를 모든 줄에 반복하거나 줄 맨 앞에 1개만 몰지 마라. (a) 정렬된 실제 차트면 그대로 전사 (b) 코드 목록만 있으면 구절별 배치 (c) 코드 자료 없으면 실제 가사 위에 원키 화성으로 구절마다 바뀌는 코드를 얹음. 가사는 항상 fetch한 실제 가사.\n" +
    "STEP 4 (출처, 정직): transcribed=정렬된 실제 차트 전사 / chords_verified=실제 코드 확인·배치 추정 / chords_derived=코드 자료 없어 실제 가사 위에 이론으로 얹음. 실제 차트를 찾으면 transcribed 우선.\n" +
